...
- **🌐 Локализация** — 3 языка: русский, English, עברית (RTL); выбор в настройках и в tinyide.toml
- **🖱️ Кастомное контекстное меню** — вместо системного меню webview: своё меню в редакторе (вырезать/копировать/вставить/форматировать) и в приложении
- **⚡ Быстрое открытие папок** — ленивое дерево файлов: папка открывается мгновенно, дети подгружаются при раскрытии (защита от циклов-джункшенов)
- **🧵 Многопоточность** — tokio (async-команды через spawn_blocking: funo, git, wasmi-плагины) + rayon (параллельный поиск по файлам search_files_parallel) — UI никогда не блокируется
...
## 🎨 Slint-версия (в разработке) — нативный UI без вебвью

**Slint** — Rust-фреймворк с GPU-рендером (Vulkan/OpenGL), декларативным UI
и современным видом (не egui 😄). Весь Rust-бэкенд (Funo, git, OAuth,
WASM-плагины) встраивается в тот же процесс.

```
slint-app/  — сборка:
  cd slint-app && cargo run --release
```

Уже есть: тёмная кометная тема (cosmic-стиль), тайтлбар, проводник
(виртуальный пример + реальная папка через rfd), вкладки, редактор,
терминал-симуляция, статус-бар. Проверено: собирается и запускается.

Дальше: реальный терминал (QProcess-подобный, через std::process),
подключение tinyide-core (C ABI): Funo, git, OAuth, плагины.
