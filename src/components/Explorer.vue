...
function openMenu(e: MouseEvent) {
  ...
  const base: (MenuItemDef | 'sep')[] = [
    ...
    'sep',
    { label: 'Refresh', run: () => store.refreshTree() },
  ];
  ...
  <div v-if="ws" class="explorer">
    <div class="panel-header">
      ...
      <div class="panel-actions">
        <button title="New File" @click="startCreate('file')"><AppIcon name="plus" :size="13" /></button>
        <button title="New Folder" @click="startCreate('dir')"><AppIcon name="folder" :size="13" /></button>
        <button title="Refresh" @click="store.refreshTree()"><AppIcon name="refresh" :size="13" /></button>
      </div>
    </div>
